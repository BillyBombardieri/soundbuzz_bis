<?php 
    ...
    if ($verifAdmin == 2 || $verifAdmin == 3) {
        ...
        $requete= $orange ->prepare("SELECT DISTINCT site_crossco FROM equinix");
        $requete->execute();
        $all_site_crossco = $requete->fetchAll(PDO::FETCH_ASSOC);

        $nbCrossco= $orange ->prepare("SELECT COUNT(serial_number) FROM equinix");
        $nbCrossco->execute();
        $nbCrossco = $nbCrossco->fetchAll(PDO::FETCH_ASSOC);
        $nbCrossco = $nbCrossco[0]["COUNT(serial_number)"];
?>
<body>
<main>
    ...
    <section class="top">
        <div class="dropbtn2">
            <a href="screenshot_png.php">Exporter</a>
        </div>
        <div class="dropdown">
            <a href="equinix.php" class="dropdown"><button class="dropbtn">Retour</button></a><br><br>
        </div>
        <br><br>
        <div>
            <h1>Nombre global de Cross Connects Equinix : <?php echo($nbCrossco) ?></b></h1>
        </div><br><br>
    </section>
</main>
</body>
...
$dataPoints2 = array();
    foreach($all_site_crossco as $site_crossco){ 
        ...
        $cuivre= $orange ->prepare("SELECT COUNT(type_connecteur_client) FROM equinix WHERE type_connecteur_client LIKE'RJ%' AND site_crossco='$site_crossco' ");
        $cuivre->execute();
        $cuivre = $cuivre->fetchAll(PDO::FETCH_ASSOC);
        $cuivre= $cuivre[0]["COUNT(type_connecteur_client)"];

        $fibre= $orange ->prepare("SELECT COUNT(type_connecteur_client) FROM equinix WHERE type_connecteur_client NOT LIKE 'RJ%' AND site_crossco='$site_crossco' ");
        $fibre->execute();
        $fibre = $fibre->fetchAll(PDO::FETCH_ASSOC);
        $fibre= $fibre[0]["COUNT(type_connecteur_client)"];
